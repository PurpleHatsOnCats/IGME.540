...
void Mesh::SetBuffers(Vertex* vertices, unsigned int vertCount, unsigned int* indices, unsigned int indexCount)
{
...
{
	Mesh::name = name;
	SetBuffers(vertices, vertCount, indices, indexCount);
}

Mesh::Mesh(const char* name, const char* filename)
{
	Mesh::name = name;
	...
	//
	// *************************************
	SetBuffers(&finalVertices[0], (int)finalVertices.size(), &finalIndices[0], (int)finalIndices.size());
}
