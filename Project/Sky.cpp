	...
	Microsoft::WRL::ComPtr<ID3D11PixelShader> ps)
{
	...
	D3D11_RASTERIZER_DESC rastDisc = {};
	rastDisc.FillMode = D3D11_FILL_SOLID;
	rastDisc.CullMode = D3D11_CULL_FRONT;
	Graphics::Device->CreateRasterizerState(&rastDisc, Sky::rasterizeState.GetAddressOf());

	D3D11_DEPTH_STENCIL_DESC stencilDisc = {};
	stencilDisc.DepthEnable = true;
	stencilDisc.DepthFunc = D3D11_COMPARISON_LESS_EQUAL;
	Graphics::Device->CreateDepthStencilState(&stencilDisc, Sky::stencilState.GetAddressOf());
}

void Sky::Draw() {
	Graphics::Context->RSSetState(rasterizeState.Get());
	Graphics::Context->OMSetDepthStencilState(stencilState.Get(), 0);

	Graphics::Context->VSSetShader(vs.Get(),0,0);
	...
	Graphics::Context->RSSetState(0); // Null (or 0) restores the default state
